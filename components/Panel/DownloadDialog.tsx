  ...
  const exportImage = async (scale: ExportSize, format: DownloadFormat) => {
    if (document && window) {
      const config = {
        ...
        width: document.getElementById("codesnip").offsetWidth * scale,
        height: document.getElementById("codesnip").offsetHeight * scale
      };
      ...
      if (format === "PNG") {
        ...
        const base64 = await domToImage.toPng(
          document.getElementById("codesnip"),
          config
        );

        link.href = base64;
        link.download = `CodeSnip ${scale - 1}x.png`;
